...
def compute_food_risk_for_year(session: Session, year: int) -> None:
    rows = (
    ...
    for row in rows:
        ...
        import_dep = float(row.food_import_dep) if row.food_import_dep is not None else 0.0
        ...
        risk_unit = _clamp_unit(
            0.6 * import_dep_norm + 0.25 * concentration_norm + 0.1 * stress_norm + 0.05 * shipping_penalty
        )
        ...
        metric = (
            ...
            .filter(
                ...
                NodeMetric.metric_code == "RISK_FOOD",
