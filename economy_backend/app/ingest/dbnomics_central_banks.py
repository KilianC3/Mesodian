"""Ingestion helper for central bank series via DB.nomics."""
...
def ingest_central_bank_series(session: Session, client: DBNomicsClient | None = None) -> None:
    ...
    for indicator in _iter_indicator_configs("central_banks"):
...
if __name__ == "__main__":  # pragma: no cover - CLI entry
    with SessionLocal() as session:
        ingest_central_bank_series(session)
        print("Ingested DB.nomics central bank payloads")
