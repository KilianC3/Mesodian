...
from app.db.models import RawAfdb
...
AFDB_BASE_URL = "https://api.afdb.org/opendata"

AFDB_SERIES: Dict[str, Dict[str, Any]] = {
    "AFDB_GDP": {
        "canonical_indicator": "AFDB_GDP_GROWTH",
        "frequency": "A",
        "countries": ["NGA", "ZAF", "EGY", "KEN", "GHA"],
    },
    "AFDB_CPI": {
        "canonical_indicator": "AFDB_CPI_YOY",
        "frequency": "A",
        "countries": ["NGA", "ZAF", "EGY", "KEN", "GHA"],
    },
}
    ...
    *,
    ...
    for _, row in df.iterrows():  # type: ignore[call-arg]
        ...
        try:
            ...
            logger.warning("Skipping AfDB row %s: %s", row, exc)
            continue
        rows.append(
            {
...
def ingest_full(
    ...
    *,
    ...
    async def _run() -> None:
        for dataset_code, cfg in AFDB_SERIES.items():
            ...
            for country_id in cfg.get("countries", []):
                if selected_countries and country_id not in selected_countries:
                    continue
                df = fetch_sdmx_dataset(AFDB_BASE_URL, dataset_code)
                store_raw_payload(
                    session,
                    RawAfdb,
